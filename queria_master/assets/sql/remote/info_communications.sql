WITH filtered AS (
    SELECT *
    FROM gbizinfo.main.mart_gbizinfo_company
    WHERE regexp_matches(
        coalesce(business_items, ''),
        '(^|[|\\-])G:'
    )
),
joined AS (
    SELECT
        c.corporate_number,
        coalesce(h.name, c.name) AS company_name,
    ...
    FROM filtered c
    LEFT JOIN houjin_bangou.main.mart_houjin_bangou h
      ON c.corporate_number = h.corporate_number
)
SELECT
    *,
    'G' AS jsic_major_code,
    '情報通信業' AS jsic_major_name,
    concat_ws('|',
