...
impl Db {
    ...
    pub fn search(&self, plan: SearchPlan, page: u32, page_size: u32) -> Result<SearchResult> {
        ...
        let sqlite_capability = sqlite_supported(&plan).map_err(str::to_string).and_then(|_| {
            ...
            if searches_overridden_phone {
                let override_count: i64 = conn
                    .query_row("SELECT count(*) FROM company_contact_overrides", [], |row| {
                        row.get(0)
                    })
                    .map_err(|error| error.to_string())?;
                if override_count > 0 {
                    return Err(
                        "ローカル電話番号の上書きを含めて検索するためDuckDBが必要です".to_string(),
                    );
                }
            }
            Ok(())
        ...
        if let Err(reason) = sqlite_capability {
            if self.search_index_path.is_some() {
                warnings.push(format!("SQLite検索索引を使用せずDuckDBで検索しました: {reason}"));
            }
        } else {
    ...
    fn validate_search_index(&self, path: &Path, runtime_conn: Option<&Connection>) -> Result<u64> {
        ...
        let runtime_path = self.runtime_path.as_ref().context("ランタイムDBがありません")?;
        ...
        if let Some(expected_generation) = metadata.get("runtime_generation_id") {
            ...
            let expected_bytes = expected_bytes
                ...
                .context("source_database_bytesが数値ではありません")?;
            if runtime_path.metadata()?.len() != expected_bytes {
                return Err(anyhow!("Runtime DBと検索索引のファイルサイズが一致しません"));
            }
            if metadata
                .get("source_database")
    ...
    fn search_sqlite_index(
        ...
        let numbers = statement
            .query_map(params_from_iter(page_params.iter()), |row| row.get::<_, String>(0))?
...
fn open_search_index(path: &Path) -> Result<SqliteConnection> {
    ...
    .with_context(|| format!("SQLite検索索引を読み取り専用で開けません: {}", path.display()))?;
...
fn hydrate_companies(connection: &Connection, corporate_numbers: &[String]) -> Result<Vec<Company>> {
...
fn migrate_company_relation(connection: &Connection, runtime_mode: bool) -> Result<()> {
    ...
    if runtime_mode {
        ...
        {
            connection.execute_batch(
                "ALTER TABLE companies_local_snapshot RENAME TO companies;",
            )?;
        }
    }
    Ok(())
...
fn industry_code_condition(value: &str) -> String {
    ...
    let (pattern, business_pattern) = if normalized.len() == 1
        && normalized.chars().all(|character| character.is_ascii_alphabetic())
    {
        (
        ...
        } else if normalized.chars().all(|character| character.is_ascii_digit()) {
...
mod tests {
    ...
    fn standalone_table_survives_runtime_view_mode_round_trip() {
        ...
        connection
            .execute_batch(
                "CREATE VIEW companies AS SELECT * FROM companies_local_snapshot;",
            )
    ...
    fn v8_sqlite_index_search_hydrates_runtime_rows_in_index_order() {
        ...
        assert_eq!(result.engine, "sqlite_fts5");
    ...
    fn sqlite_fts_query_combines_supported_filters_without_duplicate_companies() {
        ...
        let count: i64 = connection
            .query_row(&sql, params_from_iter(query.params.iter()), |row| row.get(0))
        ...
        .normalize();
        let qualified_middle_query = build_sqlite_search(&qualified_middle_plan)
            .expect("build qualified middle-code query");
