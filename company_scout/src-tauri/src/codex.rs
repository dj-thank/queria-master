use crate::models::{CodexStatus, Company, IndustryGuess, ResearchReport, ResearchSource, SearchPlan};
...
impl CodexManager {
    pub fn new(app_data_dir: PathBuf, workspace_dir: PathBuf) -> Self {
        Self { app_data_dir, workspace_dir, session: Arc::new(Mutex::new(None)) }
    }
    ...
    async fn spawn(&self) -> Result<CodexSession> {
        ...
        let stdin = child.stdin.take().ok_or_else(|| anyhow!("Codex stdinを取得できません"))?;
        let stdout = child.stdout.take().ok_or_else(|| anyhow!("Codex stdoutを取得できません"))?;
    ...
    pub async fn status(&self) -> CodexStatus {
        let session = match self.ensure().await {
            Ok(v) => v,
            Err(_) => return CodexStatus {
                running: false, authenticated: false, email: None, plan_type: None,
                luna_available: false, model: LUNA_MODEL.into(), version: None,
            },
        };
        let account = session.request("account/read", Some(json!({"refreshToken": false}))).await.ok();
        let models = session.request("model/list", Some(json!({"limit": 100, "includeHidden": true}))).await.ok();
        let account_obj = account.as_ref().and_then(|v| v.get("account"));
        let account_type = account_obj.and_then(|v| v.get("type")).and_then(|v| v.as_str());
        let authenticated = account_type == Some("chatgpt");
        let email = account_obj.and_then(|v| v.get("email")).and_then(|v| v.as_str()).map(str::to_string);
        let plan_type = account_obj.and_then(|v| v.get("planType")).and_then(|v| v.as_str()).map(str::to_string);
        let luna_available = models
            .as_ref()
            .and_then(|v| v.get("data"))
            .and_then(|v| v.as_array())
            .map(|items| items.iter().any(|m| {
                m.get("id").and_then(|v| v.as_str()) == Some(LUNA_MODEL) ||
                m.get("model").and_then(|v| v.as_str()) == Some(LUNA_MODEL)
            }))
            .unwrap_or(false);
        CodexStatus {
            running: true, authenticated, email, plan_type, luna_available,
            model: LUNA_MODEL.into(), version: session.version.clone(),
        }
    }

    pub async fn login(&self) -> Result<String> {
        let session = self.ensure().await?;
        let value = session.request("account/login/start", Some(json!({
            "type": "chatgpt",
            "useHostedLoginSuccessPage": true,
            "appBrand": "chatgpt"
        }))).await?;
        value.get("authUrl").and_then(|v| v.as_str()).map(str::to_string)
...
"#);
    ...
    pub async fn research_company(&self, company: &Company, instruction: &str) -> Result<ResearchReport> {
        self.require_luna().await?;
...
{company_json}
"#);
        let (thread_id, text) = self.run_turn(&prompt, Some(research_schema()), "medium").await?;
        let parsed: Value = parse_json_text(&text)?;
        let sources = parsed.get("sources").and_then(Value::as_array).map(|items| items.iter().filter_map(|v| {
            Some(ResearchSource {
        ...
        let findings = parsed.get("findings").and_then(Value::as_array).map(|items| items.iter().filter_map(|v| v.as_str().map(str::to_string)).collect()).unwrap_or_default();
        let industry_guess = parsed.get("industry_guess").and_then(|v| if v.is_null() { None } else { Some(v) }).map(|v| IndustryGuess {
            code: v.get("code").and_then(Value::as_str).map(str::to_string),
            name: v.get("name").and_then(Value::as_str).map(str::to_string),
            confidence: v.get("confidence").and_then(Value::as_f64),
            rationale: v.get("rationale").and_then(Value::as_str).map(str::to_string),
        });
        Ok(ResearchReport {
            ...
            transcript: parsed.get("transcript").and_then(Value::as_str).unwrap_or("").to_string(),
            industry_guess,
            findings,
            sources,
    ...
    async fn require_luna(&self) -> Result<()> {
        ...
        if !status.running { return Err(anyhow!("Codex App Serverを起動できません。Codexランタイムを確認してください。")); }
        if !status.authenticated { return Err(anyhow!("ChatGPTでログインしてください。APIキー認証はこのアプリでは許可していません。")); }
        if !status.luna_available { return Err(anyhow!("このChatGPTアカウントではGPT-5.6 Lunaを利用できません。CompanyMasterは他モデルへフォールバックしません。")); }
        Ok(())
    }

    async fn run_turn(&self, prompt: &str, output_schema: Option<Value>, effort: &str) -> Result<(String, String)> {
        let session = self.ensure().await?;
        let mut events = session.events.subscribe();
        let thread = session.request("thread/start", Some(json!({
            "model": LUNA_MODEL,
        ...
        let thread_id = thread.get("thread").and_then(|v| v.get("id")).and_then(Value::as_str)
            .ok_or_else(|| anyhow!("Codex thread/startの応答にthread.idがありません"))?.to_string();
        let mut params = json!({
            "threadId": thread_id,
        ...
        if let Some(schema) = output_schema { params["outputSchema"] = schema; }
        let turn = session.request("turn/start", Some(params)).await?;
        let turn_id = turn.get("turn").and_then(|v| v.get("id")).and_then(Value::as_str)
            .ok_or_else(|| anyhow!("Codex turn/startの応答にturn.idがありません"))?.to_string();
...
impl CodexSession {
    async fn request(&self, method: &str, params: Option<Value>) -> Result<Value> {
        ...
        if let Some(params) = params { msg["params"] = params; }
...
fn parse_json_text<T: serde::de::DeserializeOwned>(text: &str) -> Result<T> {
    ...
    if let Ok(v) = serde_json::from_str(trimmed) { return Ok(v); }
    let cleaned = trimmed
        .strip_prefix("```json").or_else(|| trimmed.strip_prefix("```"))
        .and_then(|v| v.strip_suffix("```"))
        .map(str::trim)
        .unwrap_or(trimmed);
    serde_json::from_str(cleaned).with_context(|| format!("LLMのJSONを解析できません: {}", truncate(cleaned, 400)))
}

fn truncate(value: &str, max: usize) -> String {
    if value.chars().count() <= max { value.to_string() } else { value.chars().take(max).collect::<String>() + "…" }
}
