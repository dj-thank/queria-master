...
impl PublicEnrichmentManager {
    ...
    pub async fn status(&self) -> PublicEnrichmentStatus {
            ...
            .await
        {
            Ok(value) => match serde_json::from_value::<CliStatus>(value) {
                ...
                Err(error) => status.error = Some(format!("状態JSONを解釈できません: {error}")),
            },
    ...
    pub async fn run_all(
        ...
        let result = self
            .run_public_cli(args, Duration::from_secs(2 * 60 * 60))
            .await?;
        self.operation("run-all", result).await
    }
    ...
    async fn run_public_cli(&self, args: Vec<OsString>, limit: Duration) -> Result<Value> {
        ...
        let output = timeout(limit, command.output())
            .await
            .map_err(|_| anyhow!("公開データ統合処理が制限時間を超えました"))?
            .context("公開データ統合プロセスを起動できません")?;

        if output.stdout.len() > MAX_OUTPUT_BYTES || output.stderr.len() > MAX_OUTPUT_BYTES {
            return Err(anyhow!("公開データ統合プロセスの出力が大きすぎます"));
        }
