...
impl SalesforceManager {
    pub fn new() -> Result<Self> {
        Ok(Self {
            client: Client::builder().user_agent("CompanyMaster/0.2").build()?,
    ...
    pub async fn login_start(
        ...
        login_url: &str,
        client_id: &str,
    ) -> Result<SalesforceLoginStart> {
        let login_url = normalize_login_url(login_url)?;
        let client_id = client_id.trim().to_string();
        if client_id.is_empty() {
            return Err(anyhow!("Client IDが空です"));
        }

        let listener = TcpListener::bind(OAUTH_CALLBACK)
            .await
    ...
    pub async fn upsert(
    ...
    ) -> Result<SalesforceUpsertResult> {
        if companies.is_empty() {
            return Err(anyhow!("Salesforceへ送る企業がありません"));
        }
        ...
        let create_url = format!(
            ...
            creds.instance_url, API_VERSION
        );
        ...
        if !status.is_success() {
            return Err(anyhow!("Bulk API job作成失敗 ({status}): {value}"));
        }
        let job_id = value
            .get("id")
            .and_then(Value::as_str)
            .ok_or_else(|| anyhow!("Bulk API job idがありません"))?
            .to_string();

        let csv = build_csv(companies, &mapping)?;
        let batch_url = format!(
            "{}/services/data/{}/jobs/ingest/{}/batches",
            creds.instance_url, API_VERSION, job_id
        );
        let upload = self
            .client
            .put(&batch_url)
        ...
        Ok(SalesforceUpsertResult {
            accepted: companies.len() as u64,
            job_id,
        })
    }
    ...
    pub async fn job_status(&self, job_id: &str) -> Result<SalesforceJobStatus> {
        ...
        let value: Value = response.json().await.unwrap_or(Value::Null);
        if !status.is_success() {
            return Err(anyhow!("Bulk APIジョブ状態取得失敗 ({status}): {value}"));
        }
    ...
    pub async fn retry_failed(&self, job_id: &str) -> Result<SalesforceUpsertResult> {
        let job = self.jobs.lock().await.get(job_id).cloned().ok_or_else(|| {
            anyhow!("このアプリ起動後のジョブ情報がありません。元リストから再送してください")
        })?;
        let mut creds = load_credentials().context("Salesforceに接続してください")?;
        let url = format!(
            "{}/services/data/{}/jobs/ingest/{}/failedResults",
            creds.instance_url,
            API_VERSION,
            safe_job_id(job_id)?
        );
        ...
        let key_index = headers
            .iter()
            .position(|h| h == job.external_id_field)
        ...
        for row in reader.records() {
            ...
            if let Some(value) = row.get(key_index) {
                failed_keys.insert(value.to_string());
            }
        }
        if failed_keys.is_empty() {
            return Err(anyhow!("Salesforceが返した失敗行は0件です"));
        }
        let failed_companies: Vec<Company> = job
            .companies
            .into_iter()
            .filter(|company| failed_keys.contains(&company.corporate_number))
            .collect();
        if failed_companies.is_empty() {
            return Err(anyhow!("失敗行の法人番号を元リストから特定できません"));
        }
        self.upsert(
            &failed_companies,
            &job.object_name,
            &job.external_id_field,
            &job.mapping,
        )
        .await
    }
    ...
    async fn refresh(&self, mut creds: StoredCredentials) -> Result<StoredCredentials> {
        let refresh = creds.refresh_token.clone().ok_or_else(|| {
            anyhow!("Salesforce refresh tokenがありません。再ログインしてください。")
        })?;
        ...
        let value: Value = response.json().await.unwrap_or(Value::Null);
        if !status.is_success() {
            return Err(anyhow!("Salesforce token refresh失敗 ({status}): {value}"));
        }
        creds.access_token = value
            .get("access_token")
            .and_then(Value::as_str)
            .ok_or_else(|| anyhow!("access_tokenがありません"))?
            .to_string();
        if let Some(instance) = value.get("instance_url").and_then(Value::as_str) {
            creds.instance_url = instance.to_string();
        }
...
async fn complete_oauth(
    ...
    let first = request
        .lines()
        .next()
        .ok_or_else(|| anyhow!("OAuth callbackが不正です"))?;
    let target = first
        .split_whitespace()
        .nth(1)
        .ok_or_else(|| anyhow!("OAuth callback pathがありません"))?;
    let callback = Url::parse(&format!("http://127.0.0.1{target}"))?;
    let params: std::collections::HashMap<_, _> = callback.query_pairs().into_owned().collect();
    let state = params
        .get("state")
        .ok_or_else(|| anyhow!("OAuth stateがありません"))?;
    ...
    let code = params
        .get("code")
        .ok_or_else(|| anyhow!("OAuth codeがありません"))?
        .clone();
    ...
    let instance_url = value
        .get("instance_url")
        .and_then(Value::as_str)
        .ok_or_else(|| anyhow!("instance_urlがありません"))?
        .to_string();
    let refresh_token = value
        .get("refresh_token")
        .and_then(Value::as_str)
        .map(str::to_string);
    let username = fetch_username(&client, &instance_url, &access_token)
        .await
        .ok();
...
async fn fetch_username(client: &Client, instance_url: &str, token: &str) -> Result<String> {
    let value: Value = client
        .get(format!("{instance_url}/services/oauth2/userinfo"))
        .bearer_auth(token)
        .send()
        .await?
        .json()
...
fn build_csv(companies: &[Company], mapping: &[FieldMapping]) -> Result<Vec<u8>> {
    ...
    for company in companies {
        let row: Vec<String> = mapping
            .iter()
            .map(|field| value_for_source(company, &field.source))
            .collect();
...
fn normalize_mapping(
    ...
    for field in &mut out {
        ...
        if !allowed.contains(&field.source.as_str()) {
            return Err(anyhow!("未対応のCompanyMaster項目です: {}", field.source));
        }
    }
    if !out
        .iter()
        .any(|field| field.source == "corporate_number" && field.target == external_id_field)
    {
        out.insert(
            0,
            FieldMapping {
                source: "corporate_number".to_string(),
                target: external_id_field.to_string(),
            },
        );
    }
    let mut targets = HashSet::new();
    for field in &out {
        if !targets.insert(field.target.clone()) {
            return Err(anyhow!("Salesforce項目が重複しています: {}", field.target));
        }
    }
    Ok(out)
}
...
fn default_mapping(external_id_field: &str) -> Vec<FieldMapping> {
    vec![
        ...
        FieldMapping {
            source: "corporate_number".into(),
            target: external_id_field.into(),
        },
        FieldMapping {
            source: "website".into(),
            target: "Website".into(),
        ...
        FieldMapping {
            source: "prefecture".into(),
            target: "BillingState".into(),
        },
        FieldMapping {
            source: "city".into(),
            target: "BillingCity".into(),
        },
        FieldMapping {
            source: "address".into(),
            target: "BillingStreet".into(),
        },
        FieldMapping {
            source: "industry".into(),
            target: "Industry".into(),
        },
        FieldMapping {
            source: "employees".into(),
            target: "NumberOfEmployees".into(),
        },
        FieldMapping {
            source: "business_summary".into(),
...
fn value_for_source(company: &Company, source: &str) -> String {
    match source {
        ...
        "industry" => company
            .industry_name
            .clone()
            .or_else(|| company.inferred_industry_name.clone())
            .unwrap_or_default(),
        "industry_code" => company
            .industry_code
            .clone()
            .or_else(|| company.inferred_industry_code.clone())
            .unwrap_or_default(),
        "employees" => company
            .employees
...
fn store_credentials(creds: &StoredCredentials) -> Result<()> {
    entry()?
        ...
        .map_err(|e| anyhow!("Salesforce認証情報を保存できません: {e}"))
}
...
fn normalize_login_url(value: &str) -> Result<String> {
    ...
    if url.scheme() != "https" {
        return Err(anyhow!("Salesforce Login URLはhttpsのみ許可します"));
    }
    let host = url
        .host_str()
        .ok_or_else(|| anyhow!("Salesforce Login URLが不正です"))?;
    if !(host == "login.salesforce.com"
        || host == "test.salesforce.com"
        || host.ends_with(".my.salesforce.com"))
    {
        return Err(anyhow!(
            "Salesforce login/test/My Domain URLを指定してください"
        ));
    }
    Ok(format!(
        "{}://{}{}",
