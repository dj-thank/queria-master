    ...
    <div className={`app-shell ${mobileNavOpen ? "nav-open" : ""}`}>
      <aside className="app-sidebar" aria-label="メインナビゲーション">
        <div className="brand">
          ...
          <span><strong>Queria</strong><small>Company intelligence</small></span>
        </div>
        <nav>
...
function DetailPanel({ company, busy, engine, onClose, onOpenUrl, onResearch, onCollectPhone }: {
  ...
  return (
    <aside className="detail-panel" aria-label={`${company.name}の詳細`}>
      <div className="detail-head"><div><small>法人番号 {company.corporate_number}</small><h2>{company.name}</h2></div><button className="icon-button" aria-label="詳細を閉じる" onClick={onClose}><X size={18} /></button></div>
      ...
      <dl className="detail-list">
        ...
        <div><dt>電話</dt><dd>{company.phone || "—"}</dd></div>
