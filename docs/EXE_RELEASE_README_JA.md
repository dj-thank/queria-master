...
`queria-master-desktop.exe` をダブルクリックして起動し、キーワード、地域、JSIC大・中分類、法人種別、従業員数、資本金、URL有無、件数を入力します。DesktopビルドにはTcl/Tk互換フックを含めているため、Windows環境でも同梱ランタイムを自動検出します。件数の上限は画面では1,000件です。Enterで検索し、結果をダブルクリックするとURLを開きます。表示中の結果はCSVへ書き出せます。［設定・診断］では現在のDB、索引、refresh ID、generation ID、機能可否を確認できます。
